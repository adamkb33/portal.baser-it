...
export function SimpleShinyBackground() {
  ...
  const gradientOneId = `${id}-surface-shape-1`;
  const gradientTwoId = `${id}-surface-shape-2`;
  const glowId = `${id}-surface-shape-glow`;

  return (
    <div className="pointer-events-none absolute inset-0 overflow-hidden" style={{ opacity: 'var(--opacity-background-shine)' }}>
      <svg
      ...
      >
        <defs>
          <linearGradient id={gradientOneId} x1="0%" y1="0%" x2="100%" y2="100%">
            <stop offset="0%" stopColor="var(--color-interactive)" stopOpacity="0.92" />
            <stop offset="46%" stopColor="var(--color-tertiary)" stopOpacity="0.72" />
            <stop offset="100%" stopColor="var(--color-surface-tertiary-strong)" stopOpacity="0.56" />
          </linearGradient>
          <linearGradient id={gradientTwoId} x1="100%" y1="0%" x2="0%" y2="100%">
            <stop offset="0%" stopColor="var(--color-secondary)" stopOpacity="0.88" />
            <stop offset="52%" stopColor="var(--color-tertiary)" stopOpacity="0.66" />
            <stop offset="100%" stopColor="var(--color-interactive)" stopOpacity="0.46" />
          </linearGradient>
          <filter id={glowId}>
