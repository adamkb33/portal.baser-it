...
export function Sidebar({ branches }: SidebarProps) {
  ...
  const shouldShowFocusedSection =
    !!focusedSection && !!focusedSection.node.children && focusedSection.node.children.length > 0;
...
function SidebarItem({ item, currentPath, level, onEnterSection }: SidebarItemProps) {
  ...
  const childContainerClass =
    level === 0 ? 'mt-2 space-y-1.5' : 'mt-1.5 space-y-1 border-l border-sidebar-border/50 pl-2 ml-2.5';

  return (
    <li
      role="listitem"
      className={level === 0 ? 'rounded-2xl border border-sidebar-border/70 bg-sidebar-accent/5 p-2' : ''}
    >
        ...
        >
          ...
          >
            {iconVisible ? (
              ...
              level >= 2 && (
                <span
                  className={['mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center', itemTone.bulletWrap].join(
                    ' ',
                  )}
                  aria-hidden="true"
                >
