...
export default function Appointments() {
  ...
  const availableStep = useMemo(() => {
    if (!contactId) return 'step-1';
    if (serviceIds.length === 0) return 'step-2';
    if (!userId) return 'step-3';
    if (!date || !startTime) return 'step-4';
    return 'step-5';
  }, [contactId, serviceIds.length, userId, date, startTime]);
  ...
  const groupedServices = useMemo<GroupedServiceGroupsDto[]>(() => {
    const groupsMap = new Map<
      number,
      {
        ...
        id: number;
        name: string;
        services: Map<number, GroupedServiceGroupsDto['services'][number]>;
      }
    >();

    companyUsers.forEach((user) => {
      user.services.forEach((group) => {
        if (!groupsMap.has(group.id)) {
          groupsMap.set(group.id, { companyId: group.companyId, id: group.id, name: group.name, services: new Map() });
        }
        const servicesMap = groupsMap.get(group.id)!.services;
  ...
  const eligibleCompanyUsers = useMemo(() => {
    if (serviceIds.length === 0) return companyUsers;
    return companyUsers.filter((user) =>
      serviceIds.every((serviceId) =>
        user.services.some((group) => group.services.some((service) => service.id === serviceId)),
      ),
    );
  }, [companyUsers, serviceIds]);
  ...
  const selectedServiceNames = useMemo(() => {
    ...
    return serviceIds.map((id) => serviceNameMap.get(id)).filter(Boolean) as string[];
            ...
            >
              ...
              >
                {!contactId && (
                  <div className="rounded-md border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-700">
                    Vennligst oppgi kontaktopplysninger først
                  </div>
                )}
                {contactId && groupedServices.length === 0 && (
              ...
              >
                {(!contactId || serviceIds.length === 0) && (
                  <div className="rounded-md border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-700">
                    {!contactId && 'Vennligst oppgi kontaktopplysninger først'}
                    {contactId && serviceIds.length === 0 && 'Vennligst velg minst én tjeneste først'}
                  </div>
                )}
                {contactId && serviceIds.length > 0 && (
                  <>
                    {eligibleCompanyUsers.length > 0 ? (
                      <CompanyUserPicker
                        companyUsers={eligibleCompanyUsers}
                        selectedUserId={userId}
                        onChange={handleUserChange}
                      />
                    ) : (
                      <div className="rounded-md border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-600">
                        Ingen ansatte tilbyr de valgte tjenestene
                      </div>
                    )}
                  </>
                )}
              ...
              >
                {(!contactId || serviceIds.length === 0 || !userId) && (
                  <div className="rounded-md border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-700">
                    {!contactId && 'Vennligst oppgi kontaktopplysninger først'}
                    {contactId && serviceIds.length === 0 && 'Vennligst velg minst én tjeneste først'}
                    {contactId && serviceIds.length > 0 && !userId && 'Vennligst velg en ansatt først'}
                  </div>
                )}
                {contactId && serviceIds.length > 0 && userId && (
